...
def enhance_mission_with_gemini(neighborhood, action_type, base_vibe, base_mission, active_model_name):
    """
...
BASE MISSION BLUEPRINT: {base_mission}"""
    ...
    try:
        ...
        # Self-contained parsing mechanics (replaces missing module logic dependency)
        extracted_vibe = ""
        extracted_mission = ""
        ...
        pass # Handle clean handoff to native fallback routine loops
...
if not st.session_state.started:
    ...
    # --- STEP 3: INTERACTIVE TIMELINE REORDERING (BUG STATE FIXED) ---
    if st.session_state.itinerary and not st.session_state.show_debrief:
        st.write("--------------------------------------------------")
        st.write("🔧 REORDER OR REMOVE STOPS FROM THE TIMELINE:")
        
        # Pull correct current keys to maintain options index pool alignment
...
if st.session_state.show_debrief:
    ...
    if st.button("⚡ APPEND CAMPAIGN TO SHARED CLOUD SHEET"):
        if conn is not None and not demo_mode and new_rows:
            try:
                # Explicit configuration targeting default read schema structures
